  ...
  try {
    ...
    if (!user) {
      ...
      const newUser = await User.create({
        ...
        status: "verified",
        user_password: null,
        user_id: String(nextId).padStart(6, "0"),
        otp: "000000",
      });
      const payload = {
        userId: newUser._id,
      ...
      return res
        .status(201)
        .json({ token, email: user_email, user_id: user._id });
    } else {
      const payload = {
        userId: user._id,
      };
      ...
      return res
        .status(200)
        .json({ token, email: user.user_email, user_id: user._id });
    }
  } catch (err) {
    console.log(err.message);
...
exports.verifyOtp = async (req, res) => {
  try {
    ...
    if (otpUser.status !== "unverified") {
      return res
        .status(409)
        .json({ message: "Sizning hisobingiz allaqachon tasdiqlangan" });
    }
    ...
    return res
      .status(201)
      .json({ token, email: user_email, user_id: otpUser._id });
  } catch (err) {
    console.log(err.message);
    return res.status(500).json({ message: "Serverda xatolik" });
  }
...
exports.resendOtp = async (req, res) => {
  try {
    ...
    if (user.status === "verified") {
      return res
        .status(409)
        .json({ message: "Bu foydalanuvchi allaqachon tasdiqlangan" });
    }
    ...
    return res
      .status(200)
      .json({ message: "Yangi kod yuborildi", email: user.user_email });
  } catch (err) {
    console.log(err.message);
    return res.status(500).json({ message: "Serverda xatolik" });
  }
...
exports.loginUser = async (req, res) => {
  try {
    ...
    if (!user.user_password) {
      return res
        .status(400)
        .json({ message: "Foydalanuvchi google orqali ro'yhatdan o'tgan" });
    }
